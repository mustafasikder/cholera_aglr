...
###------------- get amd prep admin shapes + cholera shape ------------------###
# prep ISO name list for the get_admin_shp function
...
## Convert cholera data as sf object
...
chl_dt_c[Country=='Ethiopia' & Admin_Level_III== 'Addis Ababa cityA', c('Admin_Level_I', 'Admin_Level_II'):= list('Addis Abeba', 'Addis Abeba')]
...
chl_dt_c[Country=='Kenya' & Admin_Level_I== 'Mandera' & Admin_Level_II== 'Kotulo', c('Admin_Level_I', 'Admin_Level_II', 'Admin_Level_III'):=   list('Mandera', 'Mandera South', 'Kutulo')]

### Admin 3
chl_dt_c[Country=='Kenya' & Admin_Level_III== 'Nairobi', c('Admin_Level_I', 'Admin_Level_II', 'Admin_Level_III'):=  list('Nairobi', 'Starehe',   'Nairobi Central')]
...
chl_dt_c[Country=='Kenya' & `Village/Town`== 'Dadaab Refugee Camp', c('Admin_Level_I', 'Admin_Level_II', 'Admin_Level_III'):=  list('Garissa', 'Daadab',   'Dadaab')]
...
################################################################################
### Unmatched admins after cleaning- should be 0 rows if all issues resolved ###
################################################################################
### Admin 1
...
chl_dt_c%>%filter(adm_level=='adm_4')


### save processed cholera data as csv
fwrite(chl_dt_c, 'results/chl_dt_w_correct_shape_name.csv')

################################################################################
###------ Merge Admin shapes with cholera data and build one sf object ------###
################################################################################
### Admin 0
# chl_adm0_sf<- merge( combined_shp_adm0, chl_dt_c[adm_level== "adm_0",], by.x= 'NAME_0', by.y= 'Country', all= T)
chl_adm0_sf<- left_join( chl_dt_c%>%filter(adm_level== "adm_0"), combined_shp_adm0, by= c('Country'= 'NAME_0'), all= T)
chl_adm0_sf<- st_as_sf(chl_adm0_sf)
nrow(chl_adm0_sf)== nrow(chl_dt_c%>%filter(adm_level=='adm_0'))
### Admin 1
...
nrow(chl_adm1_sf)== nrow(chl_dt_c%>%filter(adm_level=='adm_1'))
### Admin 2
chl_adm2_sf<- left_join( chl_dt_c%>%filter(adm_level== "adm_2"), combined_shp_adm2, by= c('Country'= 'NAME_0', 'Admin_Level_I'= 'NAME_1', 'Admin_Level_II'= 'NAME_2'), all= T)
chl_adm2_sf<- st_as_sf(chl_adm2_sf)
nrow(chl_adm2_sf)== nrow(chl_dt_c%>%filter(adm_level=='adm_2'))
### Admin 3
chl_adm3_sf<- left_join( chl_dt_c%>%filter(adm_level== "adm_3"), combined_shp_adm3, by= c('Country'= 'NAME_0', 'Admin_Level_I'= 'NAME_1', 'Admin_Level_II'= 'NAME_2', 'Admin_Level_III'= 'NAME_3'), all= T)
chl_adm3_sf<- st_as_sf(chl_adm3_sf)
nrow(chl_adm3_sf)== nrow(chl_dt_c%>%filter(adm_level=='adm_3'))
### combine all admin levels
chl_all_admin_sf<- bind_rows(chl_adm0_sf, chl_adm1_sf, chl_adm2_sf, chl_adm3_sf)

chl_all_admin_sf<- st_transform(chl_all_admin_sf, 4326)

ggplot(data= chl_all_admin_sf)+ geom_sf(aes(fill= adm_level))

saveRDS(chl_all_admin_sf, 'results/combined_shape.RDS')
st_write(chl_all_admin_sf, 'results/combined_shape.shp')
st_write(chl_all_admin_sf, 'results/combined_shape.gpkg')
